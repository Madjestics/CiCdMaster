...
public class OpenSearchExecutorLogPublisher implements ExecutorLogPublisher {
    ...
    void ensureIndexExists() {
        ...
        try {
            ...
            throw new IllegalStateException("Failed to initialize OpenSearch log index: " + index, ex);
        }
    }
    ...
    public void publish(UUID jobId, ExecutorEventMessage eventMessage) {
        if (eventMessage == null
                || eventMessage.historyId() == null
                || eventMessage.logs() == null
                || eventMessage.logs().isBlank()) {
            return;
        }

        String index = executorEventsProperties.getOpenSearch().getIndex();
        ExecutorEventDocument document = ExecutorEventDocument.logFrom(eventMessage, jobId);
        try {
            ...
            throw new IllegalStateException("Failed to publish executor logs to OpenSearch", ex);
        }
    }
}
